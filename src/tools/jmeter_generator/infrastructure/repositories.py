...
class XmlJMeterRepository(JMeterRepository):
    ...
    def _create_http_request_from_endpoint(self, endpoint: Dict[str, Any]) -> JMeterHttpRequest:
        ...
        if request_body and method in [HttpMethod.POST, HttpMethod.PUT, HttpMethod.PATCH]:
            body_data = self._generate_request_body_json(request_body, method.value)
    ...
    def _generate_request_body_json(self, request_body: Dict[str, Any], method: str = 'POST') -> str:
        """Generate JSON request body from field definitions, filtering out read-only fields."""
        if not request_body:
            return '{}'
