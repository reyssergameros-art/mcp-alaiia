...
class KarateFeatureRepository(FeatureRepository):
    ...
    def _create_scenarios_for_endpoint(self, endpoint: Dict[str, Any]) -> List[FeatureScenario]:
        ...
        if request_body and method in ['post', 'put', 'patch']:
            body_example = self._generate_request_body_example(request_body, method)
    ...
    def _generate_request_body_example(self, request_body: Dict[str, Any], method: str = 'post') -> str:
        """Generate an example request body for Karate, filtering out read-only fields."""
        if not request_body:
            return '{}'
